...
const messages = (count: number = Math.floor(Math.random() * 10)) => {
    const m: any[] = [];
    for (let i = 0; i < count; i++) {
        sample.id = Math.floor(Math.random() * 1000000000);
        m.push(sample);
    }
    return m;
};
export const TestMessageList = () => <MessageList messages={messages()} />;
